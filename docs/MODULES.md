...
`internal/notes` uses one `NTE-*` namespace for two ObjectRef types. `nte-obj` is the stable logical identity and owns the mutable current-version pointer plus soft-delete state. `version-obj` is an immutable complete Markdown snapshot with its own RefCode. Updates and old-version restores always insert a new version; restoring content never rewinds the pointer to an existing object. Both types can be resolved by Platform/ObjectRef and used as authorized LLM references.
