...
Notes
├── nte-obj (logical Note)
├── version-obj (immutable snapshot)
├── Collection
├── NoteLink
├── NoteTemplate
└── NoteSource
...
`internal/calendar` uses `EventAggregate` as the aggregate root, and `Event` is a specific schedule instance that must belong to an aggregate. Aggregates can be created empty like Accounting Accounts; Events must be created through the parent aggregate scope like Transactions. An authenticated client may also upload one textual ICS file and title; Calendar expands its bounded RFC recurrence sets and overrides into at most 512 concrete Events, then creates the aggregate and all Events atomically. Both aggregates and specific events are registered as `CAL-*` ObjectRefs and can have tags attached; metadata is immutable after creation. Specific events cannot be deleted, only transitioned via `scheduled -> finished`, `scheduled -> voided`, or `finished -> voided`; after becoming finished/voided, they do not enter the main calendar view but remain in the aggregate's sub-event list. Deletion only happens at the EventAggregate level, and the Calendar service cascades the cleanup of child events' references and tags, while writing a DELETE audit for each cascaded deleted event.

`internal/notes` uses one `NTE-*` namespace for two ObjectRef types. `nte-obj` is the stable logical identity and owns the mutable current-version pointer. `version-obj` is an immutable complete Markdown snapshot with its own RefCode. Updates always insert a new version, and the API does not provide content restoration. Deleting the logical Note permanently removes it, every version, and every associated ObjectRef in one transaction. Existing objects of both types can be resolved by Platform/ObjectRef and used as authorized LLM references.
