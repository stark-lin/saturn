...
| `internal/notes` | Logical Notes, immutable versions, collections, source sync | `nte-obj`, `version-obj`, `NoteCollection`, `NoteLink`, `NoteTemplate`, `NoteSource`, `RSSSource` | Both first-class objects share `NTE-*`; ObjectRef `object_type` distinguishes them; current title/tags/status project from the current version |
